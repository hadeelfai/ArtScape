...
app.get("/", (req, res) => {
  res.sendFile(path.resolve(__dirname, "../docs/contactus.html"));
});
...
function getFormValidation() {
  return [
    ...
    check("email")
      .isEmail()
      .withMessage("Email must follow the format x@y.z")
      .trim(),
    check("language")
      .isLength({ min: 1, max: 200 })
...
app.post("/submit-contact", getFormValidation(), async (request, response) => {
  ...
  const { firstName, lastName, gender, mobile, dob, email, language, message } =
    request.body;
