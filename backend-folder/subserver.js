...
app.get("/", (req, res) => {
  res.sendFile(path.resolve(__dirname, "../docs/submission.html"));
});
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      return res.status(400).send(`
            ...
            <p>${errors
              ...
              .map((err) => err.msg)
              .join("<br>")}</p>
            <p><a href="../docs/submission.html">Return to the form</a></p>
        `);
    }

    const { name, phone, email, artt, artc, price, year, desc, availability } =
      req.body;
    ...
    try {
      ...
      res.status(201).send(`
            ...
            <p><a href="../docs/submission.html">Submit another response</a></p>
        `);
    } catch (err) {
      res.status(500).send(`
...
app.get("/get-submission", async (req, res) => {
  ...
  try {
    const sql =
      "SELECT name, artt, artc, descr, price, year, availability FROM submissions WHERE email = ?";
    ...
    res
      .status(500)
      .json({ error: "An error occurred while retrieving the submission." });
  }
});
