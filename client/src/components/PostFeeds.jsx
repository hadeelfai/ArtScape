...
function PostFeeds({ refreshKey, onStartEditing, activeTab }) {
  ...
  const loadLoggedInUser = useCallback(async () => {
    try {
      ...
      // Fetch full user info including followingArray
  ...
  const loadPosts = useCallback(async () => {
    try {
      ...
      startTransition(() => setOptimisticPosts(filteredPosts))
  ...
  const handleLike = async (postId, currentLikesCount, isCurrentlyLiked) => {
    ...
    startTransition(async () => {
      setOptimisticPosts({ postId, liked: newLiked, likesCount: newLikesCount })
          ...
          <div className="flex-1">
            ...
            <div className="mt-3 flex items-center gap-2 text-sm text-gray-500">
              <button
                ...
                disabled={isPending}
                onClick={() =>
                  handleLike(post._id, Array.isArray(post.likes) ? post.likes.length : 0, post.isLikedByUser)
                }
              >
